...
| [`tailwind-v4-theming-with-paper.md`](theming.md) | You are designing the token model — naming, theming, responsive sizing |
| [`tailwind-v4-coverage-and-sync.md`](coverage-and-sync.md) | Something Paper cannot express, or the design and the code have drifted |
| [`skills/paper-tailwind-tokens/`](../SKILL.md) | An agent is doing the work — runtime contract, not prose |
| [`paper-mcp-normalization-tests.md`](normalization-evidence.md) | A token value came back different from what you wrote |
| ``tests/`` (repository-only reproduction) | You want to re-run the evidence yourself |
